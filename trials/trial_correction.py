...
if __name__ == '__main__':
    ...
    with pm.Model() as Model:
        ...
        T_dist = pm.TruncatedNormal('T', mu=T_guess, sigma=70, lower=0)
        T_dist_brems = pm.TruncatedNormal('T_brems', mu=T_guess_brems, sigma=40, lower=0)
        ...
        trace = pm.sample(draws=2000, tune=2000, chains=4, cores=1,step=[step_T,step_T_brems, step_noise])
        
        #derives properties from metropolis
        data_mc = pm.to_inference_data(trace) #predicted probability distribution
        df = az.summary(data_mc, round_to=4, kind="all", extend=True) #prints out prediction of true temp
        print(df)
